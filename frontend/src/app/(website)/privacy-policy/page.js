...
export async function generateMetadata() {
  const pageData = await getDynamicPage("privacy-policy");

  const title = pageData?.meta_title || pageData?.title || "Privacy Policy";
...
export default async function PrivacyPolicy() {
  ...
  const activeTitle = pageData?.title || "Privacy Policy";
          ...
          <div className="max-w-4xl mx-auto bg-white p-8 md:p-12 rounded-2xl shadow-xl border border-gray-200/80">
            {activeSections.length > 0 ? (
              ...
              <div className="text-center py-12 text-gray-500">
                Privacy policy details are currently being updated in the administrative CMS.
              </div>
            )}
